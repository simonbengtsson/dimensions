package com.chalmers.dimensions.view;
...
import com.chalmers.dimensions.model.Player;
...
public class GameView {
	...
	public GameView(GameWorld world, TiledMap mapXY, TiledMap mapXZ) {
		...
		camera.update();
		initWalkAnimation();
		batch = renderer.getBatch(); // Correct?
	}
	...
	private void drawGameObjectsXY(Batch spriteBatch) {
	...
	private void drawGameObjectsXZ(Batch spriteBatch) {
		for (GameObject gameObject : world.getGameObjects()) {
			Vector3 pos = gameObject.getPosition();
			Vector3 size = gameObject.getSize();
			spriteBatch.draw(Assets.getTexture(gameObject.getImagePath()),
