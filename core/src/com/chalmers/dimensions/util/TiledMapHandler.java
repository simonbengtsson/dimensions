...
import com.chalmers.dimensions.model.MapHandler;
...
public class TiledMapHandler implements MapHandler {
	...
	public boolean isCellGround(int x, int y) {
		return ((TiledMapTileLayer) view.getCurrentMap().getLayers()
				.get(GROUND_LAYER)).getCell(x, y) != null;
	}

	@Override
	public boolean isCellObstacle(int x, int y) {
		return ((TiledMapTileLayer) view.getCurrentMap().getLayers()
				.get(OBSTACLE_LAYER)).getCell(x, y) != null;
	}
